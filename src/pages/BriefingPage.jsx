...
function SectionLabel({ children }) {
  return (
    <div className="font-sans text-[11px] sm:text-[12px] tracking-super-wide text-sapi-gold uppercase mb-3 sm:mb-4">
      {children}
    </div>
  );
}
...
function Rule() {
  return <div className="border-t border-sapi-bronze my-8 sm:my-11" />;
}
...
export default function BriefingPage({ onBegin }) {
  ...
  return (
    <PageLayout>
            ...
            <div key={step.n} className="flex items-center gap-1 sm:gap-1.5">
              ...
              <div className="flex items-center gap-1 sm:gap-1.5">
                <div className={`w-4 h-4 sm:w-5 sm:h-5 rounded-full flex-shrink-0 flex items-center justify-center font-sans text-[11px] sm:text-[12px] font-medium ${
                  ...
                  {step.n === 1 ? (
                    <svg width="9" height="9" className="sm:w-[10px] sm:h-[10px]" viewBox="0 0 10 10" fill="none">
      ...
      {/* ── Body ── */}
      <div className="max-w-[700px] mx-auto px-4 sm:px-8 pt-10 sm:pt-14 pb-16 sm:pb-25">
        {/* Step label */}
        <div className="font-sans text-[11px] sm:text-[12px] tracking-super-wide text-sapi-gold uppercase mb-4 sm:mb-5">
          Step 2 of 3 &nbsp;·&nbsp; Assessment Briefing
        </div>

        {/* Title */}
        <h1 className="font-serif text-[26px] sm:text-[32px] font-normal text-sapi-parchment tracking-wide leading-tight mb-5 sm:mb-7">
          Your Tier 1 Sovereign AI<br />Readiness Assessment
        </h1>

        {/* Personalised greeting */}
        <div className="bg-sapi-navy border-l-[3px] border-sapi-gold px-4 sm:px-8 pt-3.5 sm:pt-4 pb-3.5 sm:pb-4 flex items-center gap-3 sm:gap-3.5 mb-8 sm:mb-11">
          {countryFlag && (
            <span className="text-xl sm:text-2xl leading-none flex-shrink-0">{countryFlag}</span>
          )}
          <div>
            <div className="font-serif text-[15px] sm:text-[17px] text-sapi-parchment tracking-wide mb-1">
        ...
        {/* ── Three-stat strip ── */}
        <div className="grid grid-cols-3 gap-2 sm:gap-3 mb-8 sm:mb-11">
          {[
            { value: "30",      label: "Questions" },
            { value: "12–15",   label: "Minutes" },
            { value: "5",       label: "Dimensions" },
          ].map(({ value, label }) => (
            <div key={label} className="bg-sapi-midnight border border-sapi-bronze border-b-2 border-b-sapi-bronze/40 px-3.5 sm:px-5 py-4 sm:py-5.5 text-center">
              <div className="font-sans text-[28px] sm:text-[34px] text-sapi-paleGold font-medium leading-none mb-2 mt-2 mb-2">
                {value}
              </div>
              <div className="font-sans text-[11px] sm:text-[12px] text-sapi-muted tracking-extra-wide uppercase mb-2">
                {label}
              </div>
            </div>
          ))}
        ...
        {/* Dimension list */}
        <div className="flex flex-col gap-px mb-8 sm:mb-11 border border-sapi-bronze">
          {DIMENSIONS.map((d, i) => (
            <div key={d.num} className={`px-3.5 sm:px-4.5 py-3 sm:py-3.5 flex items-center gap-3 sm:gap-4 ${i % 2 === 0 ? 'bg-sapi-navy' : 'bg-sapi-midnight'} ${i < DIMENSIONS.length - 1 ? 'border-b border-sapi-bronze' : ''}`}>
              <div className="font-sans text-[18px] sm:text-[22px] text-sapi-gold font-medium leading-none opacity-65 w-6 sm:w-7 flex-shrink-0 pl-1.5 sm:pl-2">
                {d.num}
              </div>
              <div className="font-serif text-[14px] sm:text-[15px] text-sapi-parchment flex-1">
                {d.name}
              </div>
              <div className="font-sans text-[12px] sm:text-[13px] text-sapi-muted tracking-wide whitespace-nowrap pr-2 sm:pr-3">
        ...
        <div className="flex flex-col gap-3.5 sm:gap-4 mb-8 sm:mb-11">
        ...
        {/* ── Confidentiality box ── */}
        <div className="border border-sapi-gold/25 border-l-[3px] border-l-sapi-gold bg-sapi-gold/5 px-4 py-3.5 sm:px-5 sm:py-4 mb-10 sm:mb-13 mt-8 sm:mt-10">
          <div className="font-sans text-[11px] sm:text-[12px] tracking-extra-wide uppercase text-sapi-gold mb-1.5 sm:mb-2">
            Data Classification
          </div>
          <p className="font-sans text-[12px] sm:text-[13px] text-sapi-muted leading-relaxed m-0">
        ...
        <div className="flex flex-col gap-3 mt-8 sm:mt-10">
          <button
            className={`w-full text-sapi-void border-none px-10 py-3.5 sm:px-12 sm:py-4 font-sans text-[12px] sm:text-[13px] tracking-extra-wide uppercase font-medium cursor-pointer rounded-sm transition-colors duration-150 ${
          ...
          <button
            className={`w-full bg-transparent border px-10 py-3 sm:px-12 sm:py-3.5 font-sans text-[12px] sm:text-[13px] tracking-extra-wide uppercase font-normal cursor-pointer rounded-sm transition-colors duration-150 ${
        ...
        <div className="font-sans text-[12px] sm:text-[13px] text-sapi-muted tracking-wide text-center mt-3 sm:mt-3.5 opacity-55 leading-relaxed">
