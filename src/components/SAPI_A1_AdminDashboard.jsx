    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <input
            type="email" value={email} placeholder="admin@sapi.ai"
          ...
          return (
            <div
              ...
              style={{
                ...
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                flexShrink: 0, transition: 'width 0.3s',
              }}
            >
              {pct > 9 && (
...
// ============================================================
// RECENT SUBMISSIONS TABLE (dashboard widget)
// ============================================================
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
        <thead>
          <tr style={{ background: '#F0EBE3' }}>
            {['Country', 'Respondent', 'Score', 'Tier', 'Stage', 'Date'].map(h => (
              <th key={h} style={{
            ...
            return (
              ...
              >
                ...
                <td style={{ padding: '9px 12px', whiteSpace: 'nowrap' }}>
                  <span style={{ color: '#EDD98A', fontFamily: 'Georgia, serif', fontSize: 15 }}>{sub.compositeScore}</span>
                  <span style={{ color: '#9880B0', fontSize: 11 }}>/100</span>
                </td>
                <td style={{ padding: '9px 12px' }}>
                  <span style={{
                    background: `${tierColor(sub.tier)}20`, color: tierColor(sub.tier),
                ...
                <td style={{ padding: '9px 12px' }}>
                  <span style={{
                    background: `${stageColor(sub.leadStage)}20`, color: stageColor(sub.leadStage),
      ...
      <div style={{ padding: '14px' }}>
        {STAGES.map((stage, i) => {
          ...
          return (
            ...
            >
              <div>
                <div style={{ fontSize: 13, fontWeight: 500, color: '#1A1A2E', lineHeight: 1.3 }}>{stage.key}</div>
...
// ============================================================
// SUBMISSIONS PAGE — full table with KPI cards
// ============================================================
function SubmissionsPage({ submissions, setAdminPage, setSelectedSubmission }) {
  const [search, setSearch] = useState('');
  const [tierFilter, setTierFilter] = useState('');
  const [stageFilter, setStageFilter] = useState('');
  const [sortCol, setSortCol] = useState('completedAt');
  ...
  const filtered = submissions
    .filter(s => {
      ...
      if (q && !s.country.toLowerCase().includes(q) && !s.respondentName.toLowerCase().includes(q) && !s.ministry.toLowerCase().includes(q)) return false;
      if (tierFilter && s.tier !== tierFilter) return false;
      if (stageFilter && s.leadStage !== stageFilter) return false;
      return true;
    })
    .sort((a, b) => {
      let av, bv;
      if (sortCol === 'compositeScore') { av = a.compositeScore; bv = b.compositeScore; }
      else if (sortCol === 'completedAt') { av = a.completedAt; bv = b.completedAt; }
      else if (sortCol === 'country') { av = shortCountry(a.country); bv = shortCountry(b.country); }
      else if (sortCol === 'respondentName') { av = a.respondentName; bv = b.respondentName; }
      else { av = a.completedAt; bv = b.completedAt; }
      if (av < bv) return -sortDir;
      if (av > bv) return sortDir;
      return 0;
    });

  // KPI calculations on filtered set
  const total = filtered.length;
  const completed = filtered.filter(s => s.upgradeStatus !== 'none' || s.leadStage !== 'New').length;
  const upgraded = filtered.filter(s => s.upgradeStatus === 'requested').length;
  const avgScore = total ? (filtered.reduce((s, r) => s + r.compositeScore, 0) / total).toFixed(1) : '—';
  ...
  const SortTh = ({ col, label, width }) => {
    ...
    return (
      ...
      >
        {label} {active ? (sortDir === 1 ? '▲' : '▼') : <span style={{ opacity: 0.3 }}>↕</span>}
      </th>
    );
  };
  ...
  return (
    <div style={{ padding: '1.75rem 2rem 2.5rem', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      ...
      <div style={{ marginBottom: '1.5rem' }}>
        ...
        <div style={{ fontSize: 12, color: '#9880B0' }}>All completed Tier 1 assessments</div>
      </div>

      {/* KPI cards */}
      <div style={{ display: 'flex', gap: 14, marginBottom: 22 }}>
        <MetricCard label="Total assessments" value={total} color="#C9963A" subtext={`of ${submissions.length} total`} />
        <MetricCard label="Avg SAPI score" value={avgScore} color="#EDD98A" subtext="composite weighted mean" />
        <MetricCard
          label="Completion rate"
        ...
        <MetricCard
          ...
          value={upgraded}
          color="#28A868"
          subtext={total ? `${Math.round(upgraded / total * 100)}% upgrade rate` : '—'}
        />
      </div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <select value={tierFilter} onChange={e => setTierFilter(e.target.value)} style={selectStyle}>
          ...
          <option>Sovereign AI Leader</option>
          <option>Advanced</option>
          <option>Developing</option>
          <option>Nascent</option>
          <option>Pre-conditions Unmet</option>
        </select>
        ...
        {(search || tierFilter || stageFilter) && (
          <button
            onClick={() => { setSearch(''); setTierFilter(''); setStageFilter(''); }}
            style={{ ...selectStyle, background: 'transparent', color: '#C9963A', border: '0.5px solid #C9963A30' }}
          >
            Clear filters
          </button>
        )}
        <span style={{ marginLeft: 'auto', fontSize: 12, color: '#9880B0' }}>
          {filtered.length} of {submissions.length} results
        </span>
      </div>
      ...
      <div style={{
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          <thead>
            <tr style={{ background: '#F0EBE3' }}>
              <SortTh col="country"       label="Country"    width="13%" />
              <SortTh col="respondentName" label="Respondent" width="20%" />
              <th style={{ padding: '8px 12px', fontSize: 10.5, fontWeight: 500, color: '#1A1A2E', textTransform: 'uppercase', letterSpacing: '0.06em', fontFamily: 'system-ui, sans-serif', width: '18%' }}>Ministry</th>
              <SortTh col="compositeScore" label="Score"     width="9%" />
              <th style={{ padding: '8px 12px', fontSize: 10.5, fontWeight: 500, color: '#1A1A2E', textTransform: 'uppercase', letterSpacing: '0.06em', fontFamily: 'system-ui, sans-serif', width: '14%' }}>Tier</th>
              {/* <th style={{ padding: '8px 12px', fontSize: 10.5, fontWeight: 500, color: '#1A1A2E', textTransform: 'uppercase', letterSpacing: '0.06em', fontFamily: 'system-ui, sans-serif', width: '11%' }}>Lead stage</th> */}
              <th style={{ padding: '8px 12px', fontSize: 10.5, fontWeight: 500, color: '#1A1A2E', textTransform: 'uppercase', letterSpacing: '0.06em', fontFamily: 'system-ui, sans-serif', width: '11%' }}>Upgrade</th>
              <SortTh col="completedAt"   label="Date"       width="10%" />
            </tr>
          </thead>
          <tbody>
            {filtered.length === 0 && (
              <tr>
                <td colSpan={8} style={{ padding: '2.5rem', textAlign: 'center', color: '#9880B0', fontSize: 13 }}>
            ...
            {filtered.map((sub, i) => {
              ...
              return (
                ...
                >
                  <td style={{ padding: '10px 12px', color: '#1A1A2E', fontWeight: 500, whiteSpace: 'nowrap' }}>
                    {shortCountry(sub.country)}
                  </td>
                  <td style={{ padding: '10px 12px' }}>
                    <div style={{ color: '#1A1A2E', lineHeight: 1.3 }}>{sub.respondentName}</div>
                    <div style={{ color: '#9880B0', fontSize: 11, marginTop: 1 }}>{sub.title.split(',')[0]}</div>
                  </td>
                  <td style={{ padding: '10px 12px', color: '#6B6577', fontSize: 12 }}>
                    {sub.ministry.length > 28 ? sub.ministry.slice(0, 28) + '…' : sub.ministry}
                  </td>
                  ...
                  <td style={{ padding: '10px 12px' }}>
                    {sub.upgradeStatus === 'requested' ? (
                      <span style={{
                        background: '#4A7AE020', color: '#4A7AE0',
                        border: '0.5px solid #4A7AE050',
                        padding: '2px 7px', borderRadius: 4,
                        fontSize: 11, fontWeight: 500,
                      }}>{sub.requestedTier}</span>
                    ) : (
                      <span style={{ color: '#C8C0B8', fontSize: 12 }}>—</span>
                    )}
                  </td>
                  <td style={{ padding: '10px 12px', color: '#6B6577', fontSize: 12, whiteSpace: 'nowrap' }}>
                    {fmtDate(sub.completedAt)}
                  </td>
                </tr>
              );
